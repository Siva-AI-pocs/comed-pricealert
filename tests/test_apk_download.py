...
downloads as pricepulse.apk, not pricepulse.zip (APKs are ZIP archives, so a
...
APK = Path(__file__).resolve().parent.parent / "app" / "static" / "downloads" / "pricepulse.apk"
...
def test_apk_served_as_android_package_with_filename(client):
    r = client.get("/static/downloads/pricepulse.apk")
    assert r.status_code == 200
    assert r.headers["content-type"] == "application/vnd.android.package-archive"
    disposition = r.headers.get("content-disposition", "")
    assert "attachment" in disposition
    assert 'filename="pricepulse.apk"' in disposition
